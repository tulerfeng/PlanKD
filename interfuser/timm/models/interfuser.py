...
from .resnet import resnet26d, resnet50d, resnet18d, resnet26, resnet50, resnet101d, resnet10
from .layers import StdConv2dSame, StdConv2d, to_2tuple

from collections import deque
from .resnet2 import *
from .resnet2 import resnet6d
...
class Interfuser(nn.Module):
    def __init__(
        ...
        if use_different_backbone:
            ...
            elif rgb_backbone_name == "r10":
                self.rgb_backbone = resnet10(
                    pretrained=False,
                    in_chans=in_chans,
                    features_only=True,
                    out_indices=[4],
                )
            elif rgb_backbone_name == "r6":
                self.rgb_backbone = resnet6d(
                    pretrained=False,
                )
            ...
            elif lidar_backbone_name == "r10":
                self.lidar_backbone = resnet10(
                    pretrained=False, in_chans=3, features_only=True, out_indices=[4]
                )
            elif lidar_backbone_name == "r6":
                # self.lidar_backbone = resnet6d(
                #     pretrained=False, in_chans=3, features_only=True, out_indices=[4]
                # )
                self.lidar_backbone = resnet6d(
                    pretrained=False,
...
@register_model 
def interfuser_baseline(**kwargs): # 52935567
    model = Interfuser(
        enc_depth=6,
        dec_depth=6,
        embed_dim=256,
...
@register_model
def interfuser_baseline2(**kwargs): #26384527
    model = Interfuser(
        enc_depth=3,
        dec_depth=3,
        embed_dim=128,
...
@register_model
def interfuser_baseline3(**kwargs): #20094575
    model = Interfuser(
        enc_depth=3,
        dec_depth=3,
        embed_dim=128,
        rgb_backbone_name="r10",
        lidar_backbone_name="r18",
        waypoints_pred_head="gru",
        use_different_backbone=True,
    )
...
@register_model
def interfuser_baseline4(**kwargs): #11674575
    model = Interfuser(
        enc_depth=3,
        dec_depth=3,
        embed_dim=64,
        rgb_backbone_name="r10",
        lidar_backbone_name="r10",
        waypoints_pred_head="gru",
        use_different_backbone=True,
    )
    return model
...
@register_model
def interfuser_baseline5(**kwargs): #3848143
    model = Interfuser(
        enc_depth=2,
        dec_depth=2,
        embed_dim=64,
        rgb_backbone_name="r6",
        lidar_backbone_name="r6",
        waypoints_pred_head="gru",
        use_different_backbone=True,
    )
    return model
