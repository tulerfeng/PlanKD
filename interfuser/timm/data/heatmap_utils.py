...
def get_actor_pos(measurements, actors_data, pixels_per_meter=5, max_distance=18):
    ego_x = measurements["x"]
    ego_y = measurements["y"]
    ego_theta = measurements["theta"]
    actor_pos = [np.array([0., 0.]) for i in range(15)]
    actor_cnt = 0
    R = np.array(
        [
            [np.cos(ego_theta), -np.sin(ego_theta)],
    ...
    for _id in actors_data:
        ...
        if (raw_loc[0] - ego_x) ** 2 + (raw_loc[1] - ego_y) ** 2 <= 1:
            ego_id = _id
            continue
        if (raw_loc[0] - ego_x) ** 2 + (raw_loc[1] - ego_y) ** 2 > 2*(max_distance)**2:
            continue
        
        new_loc = R.T.dot(np.array([raw_loc[0] - ego_x, raw_loc[1] - ego_y]))
        actor_pos[actor_cnt] = new_loc
        actor_cnt += 1
