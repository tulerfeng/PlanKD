...
class TCP(nn.Module):
    ...
    # 25.8M：self.backbone='resnet34' self.ratio=1
    # 13.9M：self.backbone='resnet18' self.ratio=2
    # 7.6M：self.backbone='resnet10' self.ratio=2
    # 3.1M：self.backbone='resnet6' self.ratio=2
    ...
    def __init__(self, config, args=None):
        ...
        if self.backbone=='resnet6':
            self.perception = resnet6(pretrained=False)
            self.bdim=256
            self.bdim2=256
        
        if self.backbone=='resnet10':
            self.perception = resnet10(pretrained=False)
        
        if self.backbone=='resnet18':
            self.perception = resnet18(pretrained=True)
        ...
        if self.backbone=='resnet50':
            ...
            self.bdim2=2176-640+512

        self.measurements = nn.Sequential(
                            nn.Linear(1+2+6, int(128//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(128//self.ratio), int(128//self.ratio)),
                            nn.ReLU(inplace=True),
                        )

        self.join_traj = nn.Sequential(
                            nn.Linear(int((128//self.ratio+self.bdim)), int(512//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(512//self.ratio), int(512//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(512//self.ratio), 256),
                            nn.ReLU(inplace=True),
                        )

        self.join_ctrl = nn.Sequential(
                            nn.Linear((int(128//self.ratio)+self.bdim2), int(512//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(512//self.ratio), int(512//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(512//self.ratio), 256),
        ...
        self.speed_branch = nn.Sequential(
                            nn.Linear(self.bdim, int(256//self.ratio)),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(256//self.ratio), int(256//self.ratio)),
                            nn.Dropout2d(p=0.5),
                            nn.ReLU(inplace=True),
                            nn.Linear(int(256//self.ratio), 1),
                        )
        ...
        self.policy_head = nn.Sequential(
                nn.Linear(256, int(256//self.ratio)),
                nn.ReLU(inplace=True),
                nn.Linear(int(256//self.ratio), 256),
                nn.Dropout2d(p=0.5),
                nn.ReLU(inplace=True),
            )
        ...
        self.init_att = nn.Sequential(
                nn.Linear(int(128//self.ratio), int(256//self.ratio)),
                nn.ReLU(inplace=True),
                nn.Linear(int(256//self.ratio), 29*8),
                nn.Softmax(1)
            )

        self.wp_att = nn.Sequential(
                nn.Linear((256+256), int(256//self.ratio)),
                nn.ReLU(inplace=True),
                nn.Linear(int(256//self.ratio), 29*8),
                nn.Softmax(1)
            )
        ...
        self.merge = nn.Sequential(
                nn.Linear(self.bdim2+256, int(512//self.ratio)),
                nn.ReLU(inplace=True),
                nn.Linear(int(512//self.ratio), 256),
            )
